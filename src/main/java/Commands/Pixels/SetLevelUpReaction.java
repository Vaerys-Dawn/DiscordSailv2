...
public class SetLevelUpReaction implements Command {
    ...
    public String execute(String args, CommandObject command) {
        MessageTokenizer messageTokenizer = new MessageTokenizer(command.message);
        ...
        if (args.equalsIgnoreCase("Remove")) {
            command.guildConfig.levelUpReaction = "null";
        ...
        if (emoji == null && customEmoji == null) {
            ...
            command.guildConfig.levelUpReaction = emoji.getUnicode();
            return "> The message a user sent to level up will now be reacted with " + emoji.getUnicode() + ".";
        } else {
            command.guildConfig.levelUpReaction = customEmoji.getName();
