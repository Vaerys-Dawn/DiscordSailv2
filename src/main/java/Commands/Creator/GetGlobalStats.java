...
import Objects.ToggleStatsObject;
...
//        }
        ...
        for (GuildContentObject g : Globals.getGuildContentObjects()) {
            CommandObject object = command.setGuild(command.client.getGuildByID(g.getGuildID()));
            for (ToggleStatsObject s : toggleStats) {
                for (GuildToggle t : object.guildToggles) {
                    if (t.name().equalsIgnoreCase(s.getToggle())) {
                        if (t.isModule()) {
                            s.addOne();
                        } else if (t.get(object.guildConfig) != t.getDefault()) {
                            s.addOne();
                        }
                    }
                }
...
//                }
//            }
            if (command.guildConfig.modulePixels) {
                for (UserTypeObject o : command.guildUsers.getUsers()) {
                    if (o.getXP() != 0) {
                        totalXP += o.getXP();
                        totalXpUsers++;
                    }
        ...
        Utility.sendDM(output.toString(), command.authorSID);
