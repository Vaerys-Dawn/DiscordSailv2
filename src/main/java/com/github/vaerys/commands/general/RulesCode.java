...
public class RulesCode extends Command {
    ...
    public String execute(String args, CommandObject command) {
        ...
        if (!Pattern.compile("\\[(.|\n)*]").matcher(command.guild.config.getRuleCode()).matches() && Pattern.compile("\\[(.|\n)*]").matcher(args).matches()) {
            command.user.queueDm("\\> That was not the right code, please try again.\n" +
            ...
            command.user.queueDm("\\> That was not the code, but you were getting close.");
            return null;
        } else {
            command.user.queueDm("\\> That was not the right code, please try again.\n");
        }
