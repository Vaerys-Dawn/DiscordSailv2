...
public class AdminCCObject {
    ...
    public boolean searchKeys(String subTag, CommandObject command) {
        for (TriVar<Long, String, Long> key : pathKeys) {
            if (key.getVar1() == command.user.longID && subTag.equalsIgnoreCase(key.getVar2())) return true;
        }
        return false;
    }
    ...
    public TriVar<Long, String, Long> getKey(String subTag, CommandObject command) {
        for (TriVar<Long, String, Long> key : pathKeys) {
            if (key.getVar1() == command.user.longID && subTag.equalsIgnoreCase(key.getVar2())) {
                return key;
            }
        }
        return null;
    ...
    public int getSlots() {
        return (contents.length() / 10000) + 1;
    }
}
